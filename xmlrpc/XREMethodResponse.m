...
//  $Id: XREMethodResponse.m,v 1.3 2003/03/28 13:12:01 znek Exp $
//
...
@implementation XREMethodResponse
...
{
    // methodResponse tags receive two kind of tags.
    // In the first case, the enclosing tag is a <fault/> tag.
    // This <fault/> tag consists of a single <value/> tag, which
    // is an XREValue in our implementation
    // In the second case, the enclosing tag is a <params/> tag.
    ...
    {
        if([[someElement valueForAttribute:@"key"] isEqualToString:@"fault"])
        {
            NSEnumerator *valuesEnum;
            XREValue *valueElement;
            
            valuesEnum = [[(XREValue *)someElement containedElements] objectEnumerator];
            while((valueElement = [valuesEnum nextObject]) != nil)
            {
                if([valueElement isKindOfClass:[XREValue class]])
                    [self takeValue:[valueElement objectValue] forAttribute:@"fault"];
            }	
        }
        else
        {
...
- (NSDictionary *)fault
{
    return [self valueForAttribute:@"fault"];
}
...
- (id)objectValue
{
    return [[self valueForAttribute:@"params"] firstObject];
}
...
- (NSString *)description
{
    ...
    if([self hasObjectValue])
        desc = [NSString stringWithFormat:@"object=\"%@\"", [self objectValue]];
    else
        desc = [NSString stringWithFormat:@"fault=\"%@\"", [self fault]];
    return [NSString stringWithFormat:@"<%@ 0x%x: %@>", NSStringFromClass([self class]), self, desc];
}
